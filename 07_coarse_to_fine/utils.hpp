...
void addShading(
    ...
    igl::parallel_for (fids.size(), [&](int id)
    // for (int id = 0; id < fids.size(); ++id)
    {
        ...
        if (fid > -1) {
            ...
            Eigen::Vector3f spec = Eigen::Vector3f::Constant(0.3f) + (0.1f * (diff.array() - 0.3f)).matrix();
            double aoFactor = ao(F(fid, 0)) * bc(0) + ao(F(fid, 1)) * bc(1) + ao(F(fid, 2)) * bc(2);

            Eigen::Vector3d pos = V3d.row(F(fid, 0)) * bc(0) + V3d.row(F(fid, 1)) * bc(1) + V3d.row(F(fid, 2)) * bc(2);
            ...
            if (flatShading) {
                ...
                norm4 = pvNormals4.row(F(fid, 0)) * bc(0) + pvNormals4.row(F(fid, 1)) * bc(1) + pvNormals4.row(F(fid, 2)) * bc(2);
            }
