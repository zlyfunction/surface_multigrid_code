...
int find_row_with_elements(const Eigen::MatrixXi &F,
...
void loop_upsample_barycentric(const Eigen::MatrixXd &V,
                               const Eigen::MatrixXi &F,
                               const int num_subdiv_iters, Eigen::MatrixXd &BC,
  ...
  for (int vIdx = 0; vIdx < SR.outerSize(); ++vIdx) {
    vector<int> adjV;
    adjV.reserve(3); // adjacent vertex indices
    vector<double> b;
    b.reserve(3); // barycentric coordinates

    for (SparseMatrix<double, Eigen::RowMajor>::InnerIterator it(SR, vIdx); it;
         ++it) {
...
int main(int argc, char *argv[]) {
  ...
  for (int ii = 0; ii < BC.rows(); ii++) {
    SV.row(ii) = BC(ii, 0) * VO.row(BF(ii, 0)) + BC(ii, 1) * VO.row(BF(ii, 1)) +
