...
void query_coarse_to_fine(const std::vector<single_collapse_data> &decInfo,
                          const Eigen::VectorXi &IM,
                          const std::vector<std::vector<int>> &decIM,
                          const Eigen::VectorXi &IMF, Eigen::MatrixXd &BC,
                          Eigen::MatrixXi &BF, Eigen::VectorXi &FIdx) {
  using namespace std;
  ...
  igl::parallel_for(
      numQuery,
      [&verbose, &FIdx, &BC, &BF, &decIM, &decInfo,
       &queryCounts](const int qIdx) {
        // // print progress
        // if (qIdx % 1 == 0 && verbose)
        ...
        while (true) {
          ...
          int v0, v1, v2; // such that subsetVIdx(v0) == f(0)
          {
            ...
            igl::find((decInfo[dIdx].subsetVIdx.array() == f(0)).eval(),
                      v0_vec);
            igl::find((decInfo[dIdx].subsetVIdx.array() == f(1)).eval(),
                      v1_vec);
            igl::find((decInfo[dIdx].subsetVIdx.array() == f(2)).eval(),
                      v2_vec);
          ...
          // get query UV
          VectorXd queryUV = BC(qIdx, 0) * decInfo[dIdx].UV_post.row(v0) +
          ...
          Eigen::MatrixXd B;
          {
            ...
            compute_barycentric(queryUV, decInfo[dIdx].UV_pre,
                                decInfo[dIdx].FUV_pre, B);
            if (verbose)
              cout << "B: \n" << B << endl;
          }
          ...
          BF(qIdx, 0) =
              decInfo[dIdx].subsetVIdx(decInfo[dIdx].FUV_pre(idxToFUV, 0));
          BF(qIdx, 1) =
              decInfo[dIdx].subsetVIdx(decInfo[dIdx].FUV_pre(idxToFUV, 1));
          BF(qIdx, 2) =
              decInfo[dIdx].subsetVIdx(decInfo[dIdx].FUV_pre(idxToFUV, 2));
