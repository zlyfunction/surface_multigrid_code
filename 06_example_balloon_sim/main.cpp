...
void runSimulation(igl::opengl::glfw::Viewer &viewer,
                   const MeshConnectivity &mesh, Eigen::MatrixXd &curPos,
                   Eigen::VectorXd &fExt, const Eigen::VectorXi &bi,
                   const Eigen::VectorXd &thicknesses, double lameAlpha,
                   double lameBeta, int matid) {
  ...
  for (int j = 0; j <= numSteps; j++) {
    ...
    if (useMG) {
      ...
      implicit_euler_mg_balloon(mesh, M, curPos, qdot, fExt, bi, edgeDOFs, *mat,
                                dt, thicknesses, abar, bbar, mg, mg_tolerance);
    } else {
      PROFC_NODE("implicit euler time");
      implicit_euler_balloon(mesh, M, curPos, qdot, fExt, bi, edgeDOFs, *mat,
                             dt, thicknesses, abar, bbar);
    }
...
int main(int argc, char *argv[]) {
  ...
  matid = 0;
  sffid = 2;
  std::string model_name = argc > 1 ? argv[1] : "bunny_15K_init";
  igl::readOBJ("../../meshes/" + model_name + ".obj", origV, F);
  ...
  menu.callback_draw_viewer_menu = [&]() {
    ...
    if (ImGui::CollapsingHeader("Optimization",
      ...
      if (ImGui::Button("Optimize Some Step", ImVec2(-1, 0))) {
        ...
        switch (sffid) {
        case 0:
          runSimulation<MidedgeAngleTanFormulation>(viewer, mesh, curPos, fExt,
                                                    bi, thicknesses, lameAlpha,
                                                    lameBeta, matid);
          break;
        case 1:
          runSimulation<MidedgeAngleSinFormulation>(viewer, mesh, curPos, fExt,
                                                    bi, thicknesses, lameAlpha,
                                                    lameBeta, matid);
          break;
        case 2:
          runSimulation<MidedgeAverageFormulation>(viewer, mesh, curPos, fExt,
                                                   bi, thicknesses, lameAlpha,
                                                   lameBeta, matid);
          break;
        default:
